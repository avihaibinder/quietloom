...
export function createRain(ctx: BaseAudioContext, { scheduler, params = {}, key = 'rain' }: LayerIO): Layer {
  ...
  const bedBuffer = getNoiseBuffer(ctx, 'white', 'rain');
...
function cutoffFor(intensity: number): number {
  return 5200 + 2400 * intensity;
}

function bedLevelFor(intensity: number): number {
  return 0.3 + 0.7 * intensity;
}

function dropLevelFor(intensity: number): number {
