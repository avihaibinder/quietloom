...
import { createBandBank, createVoicePool, getGrainBuffer, getNoiseBuffer } from '../noise';
import type { Band, BandBank } from '../noise';
import { poissonGap } from '../scheduler';
import type { Layer, LayerIO } from '../types';

const CRACKLE_BANDS: readonly Band[] = [
...
const POP_BANDS: readonly Band[] = [
  { freq: 700, q: 3, pan: -0.25 },
  { freq: 900, q: 3, pan: 0.3 },
];

export function createFire(ctx: BaseAudioContext, { scheduler, key = 'fire' }: LayerIO): Layer {
  const t0 = ctx.currentTime;
  const out = ctx.createGain();

  const bed = ctx.createBufferSource();
  const bedBuffer = getNoiseBuffer(ctx, 'brown', 'fire');
  ...
  lp.connect(bedGain);
  bedGain.connect(out);
  bed.start(t0, Math.random() * bedBuffer.duration);
  ...
  function spark(t: number, bank: BandBank, dur: number, peak: number, rateJitter: number): void {
