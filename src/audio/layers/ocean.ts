 ...
 */
function swellWave(ctx: BaseAudioContext, tStart: number, lag: number): PeriodicWave {
  return phasedSine(ctx, TAU * OCEAN_HZ * (tStart - lag) - Math.PI / 2);
}

export function createOcean(ctx: BaseAudioContext, { scheduler, key = 'ocean' }: LayerIO): Layer {
  const t0 = ctx.currentTime;
  const out = ctx.createGain();

  const src = ctx.createBufferSource();
  const srcBuffer = getNoiseBuffer(ctx, 'pink', 'ocean');
