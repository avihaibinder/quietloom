 ...
 *   - the meadow is one offscreen canvas covering the bottom band of the screen;
 ...
 *     pattern from stars.js), so repeated resizes never grow the cache.
 *
 * Everything moves off the `dt` / `env.time` the renderer hands in. This module
 * never calls requestAnimationFrame and never sets a timer: an all-night app
 * cannot afford a second clock, and the renderer already stops the only one
 * there is when the document is hidden, bedside is on, or audio is stopped.
 *
...
let sky = null; // ctx-bound linear gradient
let field = [];
let clouds = [];
let meadow = null; // offscreen canvas: the band the crest moves through
let meadowTop = 0;
let meadowBottom = 0;
let moonSprite = null;
let halo = null; // fixed-size, resolution independent
...
/**
 * Everything the UI needs to know about the moon on screen, in CSS px:
 * where it is, how big, whether the hill has swallowed it, and how much cloud
 * is currently sitting on it. The crest and the cloud maths live here so no
 * other layer has to keep a copy of them.
 */
export function moonTarget() {
...
function buildMoon(nowMs) {
  ...
  // The lit region goes onto its own canvas first so the terminator can be
  ...
  g.arc(cx, cy, moonR, 0, Math.PI * 2);
  g.clip();
  if ('filter' in g) g.filter = `blur(${Math.max(1, moonR * 0.07).toFixed(2)}px)`;
  g.drawImage(lit, 0, 0);
  g.filter = 'none';
  g.restore();
...
function ensureSky(ctx, env) {
  if (sky) return;
  sky = ctx.createLinearGradient(0, 0, 0, env.h);
  sky.addColorStop(0, '#0a1020');
  sky.addColorStop(0.55, '#060a14');
  sky.addColorStop(1, '#03040a');
}

/** The stars.js field at half density and two thirds of the alpha. */
...
function cloudSprite(w, h) {
  ...
  const lumps = document.createElement('canvas');
  lumps.width = c.width;
  lumps.height = c.height;
  const lg = lumps.getContext('2d');
  const blobs = Math.max(8, Math.round(c.width / (c.height * 0.85)));
  for (let i = 0; i < blobs; i += 1) {
    const cx = c.width * ((i + 0.5) / blobs + rand(-0.4, 0.4) / blobs);
    const cy = c.height * (0.42 + 0.16 * Math.random());
    const r = c.height * rand(0.36, 0.5);
    const grad = lg.createRadialGradient(cx, cy, 0, cx, cy, r * 2.2);
    ...
    lg.translate(-cx, -cy);
    lg.beginPath();
    lg.arc(cx, cy, r, 0, Math.PI * 2);
    lg.fill();
    lg.restore();
  }
  if ('filter' in g) g.filter = `blur(${(c.height * 0.22).toFixed(1)}px)`;
  ...
  const ends = g.createLinearGradient(0, 0, c.width, 0);
  ...
  const edges = g.createLinearGradient(0, 0, 0, c.height);
...
function buildMeadow(env) {
  ...
  const c = document.createElement('canvas');
  c.width = Math.max(1, Math.round(env.w * dpr));
...
/** @returns {boolean} whether there is a sheep to draw this frame. */
function stepSheep(env, dt) {
...
function drawSheep(ctx, env) {
  ...
  ctx.beginPath();
  ctx.ellipse(0, 0, 11.5, 7, 0, 0, Math.PI * 2);
  ctx.fill();
...
function paint(ctx, env, dt, animate) {
  ensureSky(ctx, env);
  ctx.fillStyle = sky;
  ...
  if (moonAt === env.time && new Date().getDate() !== phaseDay) buildMoon(Date.now());
  ...
  ctx.globalAlpha = 1;
  if (moonSprite) {
    ctx.drawImage(moonSprite, Math.round(m.x - moonSprite.width / 2), Math.round(m.y - moonSprite.height / 2));
  }
...
export const moonrise = {
  id: 'moonrise',
  accent: '#a9b9dd',
  accentSoft: 'rgba(169,185,221,0.16)',
  top: '#0a1020',
  bottom: '#03040a',
